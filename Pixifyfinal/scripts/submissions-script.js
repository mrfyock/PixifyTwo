const imageSets = [
  ...
  window.onload = function(){
    ...
    // get gallery that user selected from all albums
    const currentGallery = imageSets.filter(item => item.id == currentId);
    console.log(currentGallery);
    const gallery = `
    <div class="image-gallery">
        <div class="image-title">
          <h1>${currentGallery[0].title}</h1>
        </div>
        <div class="image-name">
          <h3>${currentGallery[0].name}</h3>
        </div>
        
        <img id="current-img" src=${currentGallery[0].images[0]} alt=${currentGallery[0].name}>
        
        <div class="buttons-container">
          <p class="counter">${currentImgIndex +1} of ${currentGallery[0].images.length}</p>
          <button class="gallery-button" id="previous">Previous</button>
          <button class="gallery-button"id="next">Next</button>
        </div>
      </div>`
  ...
  //next button and previous buttons  
  const currentImgElement = document.getElementById("current-img");
  const nextButton = document.getElementById("next");
  const prevButton = document.getElementById("previous");
  nextButton.addEventListener("click", () => {
    currentImgIndex++;
    if (currentImgIndex >= currentGallery[0].images.length) {
      currentImgIndex = 0;
    }
    //update the counter
    currentImgElement.src = currentGallery[0].images[currentImgIndex];
    const counter = document.querySelector(".counter");
    counter.innerHTML = `${currentImgIndex+1} of ${currentGallery[0].images.length}`;
  });
  prevButton.addEventListener("click", () =>{
    currentImgIndex--;
    if (currentImgIndex < 0) {
      currentImgIndex = currentGallery[0].images.length - 1;
    }
    currentImgElement.src = currentGallery[0].images[currentImgIndex];
    const counter = document.querySelector(".counter");
    counter.innerHTML = `${currentImgIndex+1} of ${currentGallery[0].images.length}`;
...
const toggleMode = () => {
  ...
  const galleryText = document.querySelector(".image-gallery");
