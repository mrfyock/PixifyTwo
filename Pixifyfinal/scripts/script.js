    ...
    return `<div class="submission">
              <img src="${item.images}" alt="${item.title}" class="submission-image"/>
                <div class="gallery-info">              
                  ...
                  <div>
                      <a href= "details.html?id=${item.id}"><button class="view-gallery-btn">See More</button></a>
                  </div>
                </div>     
            </div>`;
  })
...
darkmodeBtn.style.color= "#7BFFE9";
let anchors = document.getElementsByClassName("submission-info");
...
const toggleMode = () => {
 ...
 const galleryBtn = document.querySelectorAll(".view-gallery-btn"); 
 if(body.style.backgroundColor === "white"){
    ...
    for (let i = 0; i < galleryBtn.length; i++) {
      galleryBtn[i].style.color = "white";
    }; 
 }else{
    ...
    for (let i = 0; i < galleryBtn.length; i++) {
      galleryBtn[i].style.color = "black";
 };
}
}
//activate theme
...
submitBtn.addEventListener("click", (e) => {
  ...
  const urlOne = document.querySelector('input[name="url-one"]').value;
  const urlTwo = document.querySelector('input[name="url-two"]').value;
  const urlThree = document.querySelector('input[name="url-three"]').value;
  const urlFour = document.querySelector('input[name="url-four"]').value;
  const urlFive = document.querySelector('input[name="url-five"]').value;
  //create new gallery div

  const newSubmission = document.createElement("div");
  newSubmission.innerHTML = `<div class="submission">
      <img src="${urlOne}" alt="${title}" class="submission-image"/>
        <div class="gallery-info">
          <div>
            <p class="submission-info">${title} - ${name}</p>
          </div>
          <div>
  ...
  document.querySelector('input[name="url-one"]').value = '';
  document.querySelector('input[name="url-two"]').value = '';
  document.querySelector('input[name="url-three"]').value = '';
  document.querySelector('input[name="url-four"]').value = '';
  document.querySelector('input[name="url-five"]').value = '';
